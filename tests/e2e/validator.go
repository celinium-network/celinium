...
import (
	...
	cryptocodectypes "github.com/cosmos/cosmos-sdk/codec/types"
...
func (v *validator) codec() codec.Codec {
	return v.chain.encfg.Codec
}

func (v validator) txconfig() client.TxConfig {
	return v.chain.encfg.TxConfig
}

func (v validator) InterfaceRegistry() cryptocodectypes.InterfaceRegistry {
	return v.chain.encfg.InterfaceRegistry
}
...
func (v *validator) init() error {
	...
	appState, err := v.chain.ModuleBasicsGenesis()
	if err != nil {
		return err
	}
...
func (v *validator) createKeyFromMnemonic(name, mnemonic string) error {
	dir := v.configDir()
	kb, err := keyring.New(keyringAppName, keyring.BackendTest, dir, nil, v.codec())
	if err != nil {
		return err
	}
...
func (c *chain) addAccountFromMnemonic(counts int) error {
	...
	kb, err := keyring.New(keyringAppName, keyring.BackendTest, val0ConfigDir, nil, c.encfg.Codec)
	if err != nil {
		return err
	}
	...
	for i := 0; i < counts; i++ {
		...
		keyRecord, err := kb.NewAccount(name, mnemonic, "", sdk.FullFundraiserPath, algo)
		if err != nil {
			return err
		}
...
func (v *validator) buildCreateValidatorMsg(amount sdk.Coin) (sdk.Msg, error) {
	...
	keyAddr, err := v.keyRecord.GetAddress()
	if err != nil {
		return nil, err
	}
	return stakingtypes.NewMsgCreateValidator(
		sdk.ValAddress(keyAddr),
		valPubKey,
		amount,
		description,
		commissionRates,
...
func (v *validator) signMsg(msgs ...sdk.Msg) (*sdktx.Tx, error) {
	txBuilder := v.txconfig().NewTxBuilder()
	...
	bytesToSign, err := v.txconfig().SignModeHandler().GetSignBytes(
	...
	bz, err := v.txconfig().TxEncoder()(signedTx)
	if err != nil {
		return nil, err
	}

	return decodeTx(v.codec(), bz, v.InterfaceRegistry())
}
